...
1. **身份备份 UX 闭环**
   - 注册成功页已有复制/下载身份备份，本地身份删除前已有保存确认提示。
   - 仍需提供备份包可选验证流程：导出后要求用户重新导入或校验 checksum。
   - 支持修改提示词后重新加密身份备份包。
   - 更突出“丢失备份包/提示词”的不可恢复提示。

2. **消息 / Mailbox / Outbox 统一状态机**
   - 定义同一消息经 WebRTC、Mailbox、Outbox 多路径发送时的状态合并规则。
   - 定义失败分类：网络失败、节点拒绝、对方未取、解密失败、重复消息、已过期。
   - Web 已有 Outbox 定时重试、当前联系人手动重发和清理已发送队列；仍需取消发送、ack 丢失恢复和状态合并规范。

3. **同步与通知策略**
   - Web 已有登录后自动发布/收取、页面切回自动收取和手动“立即同步”。
   - 仍需定义这些触发器的优先级、节流、失败重试和用户可见状态。
   - 定义 PWA 通知权限、后台限制、同步失败提醒和低电量/离线提示。
   - 明确哪些事件应通知：好友请求、好友通过、消息到达、文件到达、同步失败。

#### P1

4. **群聊生命周期**
   - 设计退群、移除成员、群名变更、管理员权限和本地群视图冲突处理。
   - 定义 Sender Key 轮换失败、成员缺少密钥、群事件乱序时的 UI。
   - 明确历史消息策略：新人默认不可见、手动转发、重新加密范围。

5. **文件传输体验**
   - 设计上传/下载进度、取消、失败重试、过期和存储空间提示。
   - 定义缩略图、预览、文件名/MIME 展示和危险文件警告。
   - 明确大文件是否分片、是否支持断点续传。

6. **本地数据管理**
   - 设计会话删除、联系人删除、清理缓存、存储占用展示。
   - 设计本地搜索如何在加密字段上工作。
   - 设计完整数据备份恢复后的冲突处理。
...
7. **反滥用 UX**
   - 陌生请求频率限制、批量拉黑、垃圾请求隔离区。
   - 本地过滤器配置和误拦截恢复。
   - 明确“不提供全局举报/审核”时的用户说明。

8. **诊断报告规范**
   - Web 已有诊断页和状态摘要报告，并声明不导出提示词、身份私钥或消息明文。
   - 仍需把允许字段/禁止字段写成规范，覆盖网络、节点、存储、版本信息和最近日志。
   - 设计用户分享诊断报告给开发者前的预览、脱敏和确认。

9. **可访问性与国际化**
   - 键盘可用性、屏幕阅读器标签、颜色对比度。
   - 错误文案、时间格式、文件大小和语言包边界。
