...
- Output of `./scripts/risk-register-gate.sh` proving every non-low residual risk has an owner, evidence, and release decision; `./scripts/preprod-evidence.sh` archives report-mode output as `risk-register-gate.log` and records machine-readable `production_gate.risk_register_gate_status` in `release-evidence-index.json`.
