...
Before production sign-off, run the risk-register gate. It is intentionally not part of quick CI because open risks are allowed during development, but it is a no-go gate for production release:
...
The evidence collector will archive `release-asset-verify-report.json` alongside the normal release-check, fuzz, SQLCipher, federation, and risk-register gate reports.
...
- Output of `./scripts/risk-register-gate.sh` proving every non-low residual risk has an owner, evidence, and release decision; `./scripts/preprod-evidence.sh` archives report-mode output as `risk-register-gate.log`.
