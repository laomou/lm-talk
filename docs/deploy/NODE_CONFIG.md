...
| `state_db_encryption_mode` | string | `plain` | `plain` 表示未加密 SQLite；`external` 表示由外部磁盘/文件系统加密承担保护；`sqlcipher` 为数据库级加密 provider。SQLCipher 已默认编入发布二进制（rusqlite/libsqlite3-sys 的 bundled SQLCipher + vendored OpenSSL），选 `sqlcipher` 模式并提供口令后执行 `PRAGMA key` 并校验 `PRAGMA cipher_version`。 |
