...
- 自动 peer snapshot sync 的配置 schema 文档；后续将 control-peer DHT replication scaffold 升级为按 closest-k target 选择远端，并为开放传输层返回节点补充可携带 identity public key 的端到端签名校验。
