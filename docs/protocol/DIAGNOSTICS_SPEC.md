...
- 只允许计数，不允许导出联系人列表、群成员列表、消息列表、隔离请求明细、去重 ID 列表或附件元数据列表。
...
- 单条日志导出前必须先脱敏再裁剪长度，避免长原文或完整载荷进入报告。当前 Web 会脱敏 Bearer token、`http://...|token` 形式同步服务令牌、所有匹配 `lm-*-vN:` 的协议原文和 JSON 中的敏感字段。
...
- 当前 Web 会对诊断状态和最近日志执行敏感文本脱敏，并有 E2E 覆盖 Bearer token、URL token 和身份备份包前缀不会进入诊断 JSON。
...
诊断报告可以导出 PWA 后台事件计数、最近后台事件标签和时间摘要，用于排查 Background Sync / Periodic Sync / Push 是否触发。不得导出 Service Worker 中不存在的消息内容、密钥、Mailbox payload 或通知正文之外的敏感数据。
