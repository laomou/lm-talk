...
| `state_db_encryption_mode` | string | `plain` | `plain` 表示未加密 SQLite；`external` 表示由外部磁盘/文件系统加密承担保护；`sqlcipher` 为数据库级加密 provider 预留；默认构建会拒绝，`lm_node --features sqlcipher` 目前只启用 fail-closed 骨架，尚未完成 SQLCipher 连接初始化。 |
| `state_db_passphrase_file` | string | - | 预留给 DB 级加密 provider 的口令文件入口；当前 plain/external 模式不会使用该口令，但启动时会按 secret file 权限规则读取并注入 provider 边界；`sqlcipher` feature 骨架会要求该口令但仍 fail-closed。 |
