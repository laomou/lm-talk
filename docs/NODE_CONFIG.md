...
| `state_db_encryption_mode` | string | `plain` | `plain` 表示未加密 SQLite；`external` 表示由外部磁盘/文件系统加密承担保护；`sqlcipher` 为数据库级加密 provider 预留；默认构建会拒绝，`lm_node --features sqlcipher` 会执行 SQLCipher `PRAGMA key` 并校验 `PRAGMA cipher_version`，若当前链接的 SQLite 不支持 SQLCipher 会 fail-closed。 |
| `state_db_passphrase_file` | string | - | 预留给 DB 级加密 provider 的口令文件入口；当前 plain/external 模式不会使用该口令，但启动时会按 secret file 权限规则读取并注入 provider 边界；`sqlcipher` feature 会要求该口令并通过 provider 传给 `PRAGMA key`；若运行时库不是 SQLCipher 会 fail-closed。 |
