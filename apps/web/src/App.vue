...
const activeSessionRecoveryState = computed(() => {
  const contact = activeContact.value
  if (!contact) return 'none' as const
  const sessionFailures = mailboxFailedItems.value.filter((item) =>
    String(item.message?.from_user_id ?? '') === contact.user_id
    && mailboxFailureCategory(item.reason) === 'session',
  )
  if (!sessionFailures.length) return 'none' as const
  if (pendingSecureSessionOfferFor(contact.user_id)) return 'waiting' as const
  const newestFailureAt = Math.max(...sessionFailures.map((item) => item.last_failed_at))
  if ((contact.last_secure_session_success_at ?? 0) >= newestFailureAt) return 'recovered' as const
...
const activeStrictE2eeSendBlockingText = computed(() => {
  if (!activeContact.value || activeContact.value.state !== 'Friend') return ''
  const reasons = strictE2eeSendBlockingReasons(activeContact.value)
  return reasons.length ? `严格 E2EE 将阻止发送：${reasons.join('；')}` : ''
})
...
function contactStrictE2eeAdvisoryReasons(contact: ContactItem): string[] {
  if (contact.state !== 'Friend') return []
  const reasons: string[] = []
  if (contactCardDhtDiscoveryIsStale(contact)) reasons.push('ContactCard DHT 信息未刷新')
  const ack = contactCardUpdateAckStatusFor(contact)
  if (ack.pending) reasons.push(`设备更新待确认 ${ack.pending} 条`)
  return reasons
}

function contactStrictE2eeStatus(contact: ContactItem): {
  level: 'ok' | 'advisory' | 'blocking' | 'none'
  label: string
  detail: string
} {
  if (contact.state !== 'Friend') return { level: 'none', label: '', detail: '' }
  ...
  const advisories = contactStrictE2eeAdvisoryReasons(contact)
...
function contactStrictE2eeStatusText(contact: ContactItem): string {
  const status = contactStrictE2eeStatus(contact)
  return status.level === 'none' ? '' : `${status.label}：${status.detail}`
}

function contactStrictE2eeRiskLevel(contact: ContactItem): 'ok' | 'warning' | 'high' | 'none' {
  return contactStrictE2eeStatus(contact).level === 'blocking'
    ? 'high'
    : contactStrictE2eeStatus(contact).level === 'advisory'
      ? 'warning'
      : contactStrictE2eeStatus(contact).level === 'ok'
        ? 'ok'
        : 'none'
}
...
const strictE2eeReadiness = computed(() => {
  ...
  return {
    ...
    text: blockers.length
      ? `严格 E2EE 核心条件未满足：${blockers.join('；')}`
      : advisories.length
        ? `严格 E2EE 核心条件已满足；建议刷新：${advisories.join('；')}`
        : '严格 E2EE 核心条件已满足。',
  }
})

const strictE2eeReadinessSummaryReportText = computed(() => JSON.stringify({
  type: 'lm-strict-e2ee-readiness-summary-v1',
  generated_at: new Date().toISOString(),
  strict_policy_enabled: strictE2eePolicyEnabled.value,
  readiness: strictE2eeReadiness.value,
  contact_counts: {
    friends: friendContacts.value.length,
    verified: verifiedFriendContactCount.value,
    unverified: unverifiedFriendContactCount.value,
    sealed_slot_risk: sealedSlotRiskContacts.value.length,
    stale_contact_card_dht: friendContacts.value.filter((contact) => contactCardDhtDiscoveryIsStale(contact)).length,
    pending_contact_update_acks: contactCardUpdatePendingAckCount.value,
  },
  group_counts: {
    total: groups.value.length,
    strict_e2ee_risk: groups.value.filter((group) => groupStrictE2eeRiskTextFor(group)).length,
  },
  issue_counts: strictE2eeReadinessIssues.value.reduce((acc: Record<string, number>, item: any) => {
    const key = item.issue_kind || 'unknown'
    acc[key] = (acc[key] || 0) + 1
    return acc
  }, {}),
}, null, 2))

const strictE2eeReadinessReportText = computed(() => JSON.stringify({
  type: 'lm-strict-e2ee-readiness-report-v1',
  generated_at: new Date().toISOString(),
  strict_policy_enabled: strictE2eePolicyEnabled.value,
  readiness: strictE2eeReadiness.value,
  contact_counts: {
  ...
  risky_contacts: friendContacts.value
    .filter((contact) => contactStrictE2eeRiskLevel(contact) === 'high')
    .slice(0, 16)
    .map((contact) => ({
      user_id: contact.user_id,
      display_name: contact.display_name || '',
      status: contactStrictE2eeStatusText(contact),
      last_contact_card_dht_found_at: contact.last_contact_card_dht_found_at ?? null,
    })),
  risky_groups: groups.value
    .filter((group) => groupStrictE2eeRiskTextFor(group))
    .slice(0, 16)
    .map((group) => ({
      group_id: group.group_id,
      name: group.name,
      member_count: group.member_user_ids.length,
      status: groupStrictE2eeRiskTextFor(group),
    })),
}, null, 2))

async function copyStrictE2eeReadinessReport() {
  await copyText(strictE2eeReadinessReportText.value, '严格 E2EE 预检报告')
}

async function copyStrictE2eeReadinessSummaryReport() {
  await copyText(strictE2eeReadinessSummaryReportText.value, '脱敏严格 E2EE 预检摘要')
}

function downloadStrictE2eeReadinessReport() {
  downloadText(strictE2eeReadinessReportText.value, `lm-strict-e2ee-readiness-${Date.now()}.json`)
  appendLog('✅ 已下载严格 E2EE 预检报告')
}

function downloadStrictE2eeReadinessSummaryReport() {
  downloadText(strictE2eeReadinessSummaryReportText.value, `lm-strict-e2ee-readiness-summary-${Date.now()}.json`)
  appendLog('✅ 已下载脱敏严格 E2EE 预检摘要')
}
...
async function retryContactSecurityUpdates(contact: ContactItem): Promise<{ pending: number; resent: number; errors: string[] }> {
  const pending = contactCardUpdateFanoutRecords.value
    .filter((record) => record.peer_user_id === contact.user_id && record.status !== 'acked')
  let resent = 0
  const errors: string[] = []
  for (const record of pending) {
    ...
    try {
      ...
      errors.push(userFacingError(error))
    }
  }
  return { pending: pending.length, resent, errors }
}

function showContactSecurityRefreshResult(contact: ContactItem, dhtError: string, updateErrors: string[]) {
  const status = contactStrictE2eeStatus(contact)
  const errors = [...(dhtError ? [dhtError] : []), ...updateErrors]
  if (status.level === 'blocking') {
    showAlert(t('settingsView.securityStillBlockedTitle'), `仍无法发送：${status.detail}`, 'error')
    return
  }
  if (errors.length) {
    showAlert(
      t('settingsView.securityInfoPartialTitle'),
      `部分安全信息暂时无法刷新：${errors.join('；')}。这不影响已有聊天；${status.level === 'advisory' ? status.detail : '可稍后重试。'}`,
      'warning',
    )
    return
  }
  showAlert(
    t('settingsView.securityInfoRefreshedTitle'),
    status.level === 'advisory'
      ? `核心安全条件已满足，聊天不受影响；${status.detail}`
      : t('settingsView.securityInfoRefreshedMessage'),
    'success',
  )
}

async function refreshContactSecurityInfoForActiveContact() {
  if (!activeContact.value || activeContact.value.state !== 'Friend') return
  const contact = activeContact.value
  let dhtError = ''
  try {
    if (contactCardDhtDiscoveryIsStale(contact)) {
      await refreshContactCardDhtForContact(contact, { preserveUi: true })
    }
    const updateResult = await retryContactSecurityUpdates(contact)
    persist()
    appendLog(`当前联系人安全信息刷新完成：ContactCard DHT ${dhtError ? '失败' : '已刷新/无需刷新'}，重发设备更新 ${updateResult.resent}/${updateResult.pending}`)
    showContactSecurityRefreshResult(contact, dhtError, updateResult.errors)
  } catch (error) {
    dhtError = userFacingError(error)
    const updateResult = await retryContactSecurityUpdates(contact)
    persist()
    appendLog(`当前联系人安全信息刷新部分失败：${dhtError}`)
    showContactSecurityRefreshResult(contact, dhtError, updateResult.errors)
  }
}

async function repairStrictE2eeForActiveContact() {
  if (!activeContact.value || activeContact.value.state !== 'Friend') return
  const contact = activeContact.value
  let dhtError = ''
  let ownCertCreated = false
  try {
    enableStrictE2eePolicy()
    ownCertCreated = await ensureOwnDeviceCertForStrict('当前联系人安全状态修复')
    if (activeContactSealedSlotRiskFor(contact) === 'high') {
      await refreshContactCardDhtForContact(contact, { preserveUi: true })
    }
  } catch (error) {
    dhtError = userFacingError(error)
  ...
  persist()
  const status = contactStrictE2eeStatus(contact)
  appendLog(`当前联系人安全状态修复完成：本设备证书 ${ownCertCreated ? '已更新' : '无需更新'}，ContactCard DHT ${dhtError ? '失败' : '已刷新/无需刷新'}，重发设备更新 ${updateResult.resent}/${updateResult.pending}`)
  if (status.level === 'blocking') {
    showAlert(t('settingsView.securityStillBlockedTitle'), `仍无法发送：${status.detail}${dhtError ? `；刷新失败：${dhtError}` : ''}`, 'error')
    return
  }
  if (dhtError || updateResult.errors.length) {
    showContactSecurityRefreshResult(contact, dhtError, updateResult.errors)
    return
  }
  showAlert(
    t('settingsView.securityRepairedTitle'),
    status.level === 'advisory'
      ? `可以发送消息。${status.detail}`
      : '可以发送消息。',
    'success',
  )
}

async function refreshStrictE2eeSecurityInfo() {
  const contactsToRefresh = friendContacts.value.filter((contact) => contactStrictE2eeStatus(contact).level === 'advisory')
  if (!contactsToRefresh.length) {
    showAlert(t('settingsView.securityInfoUpToDateTitle'), t('settingsView.securityInfoUpToDateMessage'), 'success')
    return
  }
  let refreshed = 0
  const errors: string[] = []
  for (const contact of contactsToRefresh) {
    try {
      if (contactCardDhtDiscoveryIsStale(contact)) {
        await refreshContactCardDhtForContact(contact, { preserveUi: true })
      }
      const updateResult = await retryContactSecurityUpdates(contact)
      refreshed += 1
      errors.push(...updateResult.errors.map((message) => `${contact.display_name || contact.user_id}：${message}`))
    } catch (error) {
      errors.push(`${contact.display_name || contact.user_id}：${userFacingError(error)}`)
    }
  }
  persist()
  appendLog(`批量刷新联系人安全信息完成：${refreshed}/${contactsToRefresh.length}`)
  if (errors.length) {
    showAlert(t('settingsView.securityInfoPartialTitle'), `已刷新 ${refreshed}/${contactsToRefresh.length} 个联系人；${errors.slice(0, 3).join('；')}。不影响已有聊天。`, 'warning')
    return
  }
  showAlert(t('settingsView.securityInfoRefreshedTitle'), `已刷新 ${refreshed} 个联系人；聊天不受影响。`, 'success')
}

async function repairStrictE2eeForActiveGroup() {
  ...
  await runAsync('修复当前群聊严格 E2EE', async () => {
    ...
    const members = group.member_user_ids
      .map((userId) => contacts.value.find((contact) => contact.user_id === userId))
      .filter((contact): contact is ContactItem => Boolean(contact && contact.state === 'Friend'))
    for (const contact of members) {
      ...
      if (activeContactSealedSlotRiskFor(contact) === 'high' || contactCardDhtDiscoveryIsStale(contact)) {
        try {
          ...
          appendLog(`群聊严格 E2EE 修复：${contact.display_name || contact.user_id} DHT 刷新失败：${userFacingError(error)}`)
        }
      }
      const pending = contactCardUpdateFanoutRecords.value.filter((record) => record.peer_user_id === contact.user_id && record.status !== 'acked')
      for (const record of pending) {
        record.retry_count = Number(record.retry_count ?? 0) + 1
        record.last_retry_at = Date.now()
        await sendContactCardUpdateToContact(contact)
...
function userFacingError(e: unknown): string {
  ...
  if (status === 413 || /too large|payload too large|payloadtoolarge|载荷过大|配额/i.test(raw)) return `同步节点拒绝大载荷${urlHint}：消息、附件或 Mailbox 数据超过限制，请缩小内容或检查节点配额。`
...
async function showQr(value: string, label: string) {
  try {
    ...
    qrDataUrl.value = await QRCode.toDataURL(value, {
      ...
      margin: 2,
      // Contact cards are longer than ordinary URLs. Keep enough source
      // pixels for camera and photo scanners even though CSS constrains display.
      width: 720,
    })
...
type RatchetEncryptWorkerResponse = WorkerRpcResponse & {
  state_text?: string
  envelope_json?: string
}
...
const ratchetEncryptRpc = new WorkerRpcClient<
  ...
  RatchetEncryptWorkerResponse
>(() => new Worker(new URL('./ratchetEncrypt.worker.ts', import.meta.url), { type: 'module' }), 'Ratchet 加密 Worker')

async function encryptRatchetEnvelopeInWorker(stateText: string, conversationId: string, text: string): Promise<{ stateText: string; envelopeJson: string }> {
  const response = await ratchetEncryptRpc.request({ stateText, conversationId, text })
  if (response.ok && response.state_text !== undefined && response.envelope_json !== undefined) {
    return { stateText: response.state_text, envelopeJson: response.envelope_json }
  }
  throw new Error(response.error || 'Ratchet 加密 Worker 处理失败')
}
...
function recoverSecureSessionForContact(contact: ContactItem) {
  ...
  if (contact.last_secure_session_attempt_at && Date.now() - contact.last_secure_session_attempt_at < SECURE_SESSION_RECOVERY_COOLDOWN_MS) return
...
function retrySecureSessionForActiveContact() {
  run('重试安全建链', () => {
    ...
    const contact = activeContact.value
    if (pendingSecureSessionOfferFor(contact.user_id)) {
      toast('正在等待对方的安全会话响应', 'info')
      return
    }
    if (contact.last_secure_session_attempt_at && Date.now() - contact.last_secure_session_attempt_at < SECURE_SESSION_RECOVERY_COOLDOWN_MS) {
      toast('安全会话恢复已在进行中，请稍候', 'info')
      return
    }
    recoverSecureSessionForContact(contact)
    toast('已发起安全会话恢复', 'success')
  })
}
...
async function discardUnrecoverableSessionFailuresForActiveContact() {
  if (!activeContact.value) return
  const contact = activeContact.value
  ...
  const confirmed = await showConfirm(
    '忽略无法恢复的旧消息',
    `这些消息来自恢复前的安全会话，无法用新会话解密。将从节点确认并不再重复提示；请让对方重发。确定忽略 ${items.length} 条消息吗？`,
    true,
  )
  if (!confirmed) return
  const deliveryIds = items.map((item) => item.delivery_id).filter((id): id is string => Boolean(id))
  if (deliveryIds.length) await acknowledgeMailboxDeliveries(deliveryIds)
  for (const item of items) {
    rememberProcessedMailboxIds(mailboxDedupeIds(item.delivery_id, item.message_id))
  }
  mailboxFailedItems.value = mailboxFailedItems.value.filter((item) => !items.some((failed) => failed.id === item.id))
  mailboxInboxStatus.value = `已忽略 ${items.length} 条恢复前的旧消息，请对方重发`
  mailboxInboxErrorText.value = ''
  mailboxFailureSummaryText.value = summarizeMailboxFailures(mailboxFailedItems.value.map((item) => item.reason))
  appendLog(`已忽略 ${contact.display_name || contact.user_id} 的 ${items.length} 条恢复前 Ratchet 消息`)
  persist()
}
