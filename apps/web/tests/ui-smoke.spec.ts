...
async function installMockSyncNode(context: BrowserContext, mailboxes: Map<string, MockMailboxMessage[]>) {
  ...
  await context.route('**/*', async (route) => {
    ...
    if (url.pathname === '/dht/find-value') return route.fulfill({ json: { key: url.searchParams.get('key') || 'b'.repeat(64), found: true, record: { kind: 'PreKey', value: 'lm-prekey-bundle-v1:mock-dht-prekey' }, records: 1, stats: { attempts: 2, successes: 2, failures: 0, found_records: 1, closer_records: 0, peers_quarantined: 0 } } })
