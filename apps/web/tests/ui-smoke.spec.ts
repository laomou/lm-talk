...
test('登录注册、主界面和诊断页是产品化 UI', async ({ page }) => {
  ...
  await page.goto('/#/me')
  await expect(page.getByText(/严格 E2EE 策略：已启用/)).toBeVisible()
  await page.goto('/#/chat')
...
test('消息同步可完成好友请求和消息收发', async ({ browser }) => {
  ...
  await alice.locator('.contact').filter({ hasText: 'Bob' }).click()
  await expect(alice.getByText('严格 E2EE 修复向导')).toBeVisible()
  await expect(alice.getByText('1. 身份指纹核验')).toBeVisible()
  await expect(alice.getByText('2. ContactCard DHT 新鲜度')).toBeVisible()
  ...
  await expect(alice.getByRole('heading', { name: '发送前严格 E2EE 风险提示' })).toBeVisible()
  await alice.getByRole('button', { name: '确定' }).click()
  ...
  await alice.goto('/#/contacts')
  await alice.getByRole('button', { name: '建群' }).click()
  await alice.getByLabel('群名').fill('严格 E2EE 群聊')
  await alice.getByLabel('联系人信任筛选').selectOption('all')
  await alice.getByLabel('Bob').check()
  await expect(alice.getByText('新建群聊严格 E2EE 预检', { exact: true })).toBeVisible()
  await expect(alice.locator('.callout.warning').filter({ hasText: '新建群聊严格 E2EE 预检' })).toContainText(/指纹未核验|sealed slot 风险|ContactCard DHT 未刷新|设备更新待确认/)
  // 群聊创建前已展示 strict E2EE 风险；正式创建会再次弹出浏览器确认。
  await bob.goto('/#/contacts')
  await bob.getByRole('button', { name: '同步' }).click()
  await bob.goto('/#/chat')
  await bob.locator('.contact').filter({ hasText: 'Alice' }).click()
  ...
  await expect(alice.getByRole('heading', { name: '发送前严格 E2EE 风险提示' })).toBeVisible()
  await alice.getByRole('button', { name: '确定' }).click({ force: true })
