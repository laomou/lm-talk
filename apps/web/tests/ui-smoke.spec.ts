...
async function createIdentity(page: Page, name: string, passphrase: string) {
  ...
  await expect(page.getByText('下载或复制身份文件。')).toBeVisible()
...
test('登录注册、主界面和诊断页是产品化 UI', async ({ page }) => {
  ...
  await expect(page.getByText('第一次使用建议先完成下面 3 步')).toBeVisible()
  await expect(page.getByText('1. 备份身份')).toBeVisible()
  await expect(page.getByText('2. 添加好友')).toBeVisible()
  await expect(page.getByText('3. 配置同步')).toBeVisible()
  await page.goto('/#/chat')
  await expect(page.getByText('建议开启严格 E2EE')).not.toBeVisible()
...
test('消息同步可完成好友请求和消息收发', async ({ browser }) => {
  ...
  await bob.getByRole('tab', { name: '联系人' }).click()
  ...
  await expect(bob.getByRole('heading', { name: '确认联系人指纹已核验' })).toBeVisible()
  ...
  await alice.getByRole('tab', { name: '联系人' }).click()
  await alice.locator('.contact').filter({ hasText: 'Bob' }).click()
  await alice.getByRole('button', { name: '已人工核验，标记可信' }).click()
  await expect(alice.getByRole('heading', { name: '确认联系人指纹已核验' })).toBeVisible()
...
test('设置页可导出并导入完整数据备份恢复本地同步设置', async ({ page }) => {
  ...
  await enableSync(page)
  await page.goto('/#/me')
  await page.getByRole('button', { name: '显示备份' }).click()
...
test('窄屏空聊天页从顶部展示选择聊天和首次使用指引，切回空状态会重置滚动位置', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 })
  await clearBrowserState(page)
  await createIdentity(page, 'EmptyChat', 'empty chat layout 2026')

  const emptyState = page.locator('.chat-empty-state.onboarding-empty')
  await expect(emptyState.getByRole('heading', { name: '选择一个聊天' })).toBeVisible()
  await page.locator('.messages').evaluate((element) => { element.scrollTop = 120 })
  await page.goto('/#/me')
  await page.goto('/#/chat')
  await expect(emptyState.getByRole('heading', { name: '选择一个聊天' })).toBeVisible()
  const positions = await emptyState.evaluate((element) => {
    const heading = element.querySelector('h2')!
    const messages = element.closest<HTMLElement>('.messages')!
  ...
  expect(positions.titleTop - positions.stateTop).toBeLessThan(40)
  expect(positions.messageScrollTop).toBe(0)
  expect(positions.messageScrollHeight).toBeGreaterThanOrEqual(positions.messageClientHeight)
})
