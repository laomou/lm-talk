...
test('消息同步可完成好友请求和消息收发', async ({ browser }) => {
  ...
  await expect(bob.getByText(/节点健康：.*Mailbox/)).toBeVisible()
  const bobIdentity = await localIdentityRecord(bob)
  await bob.getByLabel('DHT key 输入值').fill(bobIdentity.user_id)
  await bob.getByRole('button', { name: '派生 key' }).click()
  await expect(bob.getByLabel('DHT record key')).toHaveValue('b'.repeat(64))
  await bob.getByRole('button', { name: '查找 DHT 记录' }).click()
  await expect(bob.getByText(/DHT 查找：找到，peer 尝试 2，成功 2/)).toBeVisible()
