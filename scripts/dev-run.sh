...
Web options:
  ...
  --root-cert PATH         Export Caddy internal root certificate to this host path
...
run_web() {
  ...
  if [[ -z "$root_cert" ]]; then
    root_cert="$(dirname "$caddy_data_dir")/lm-talk-local-root.crt"
  fi
