...
python3 - <<'PY' "$OUT_DIR/release-evidence-index.json" "$VERSION" "$COMMIT" "$STARTED_AT" \
  "$release_check_present" "$release_asset_verify_present" "$risk_register_gate_present" "$risk_register_gate_report_present" "$signing_report_count"
import json, pathlib, sys
(out, version, commit, started_at, release_check, release_asset_verify, risk_register_gate, risk_register_gate_report, signing_report_count) = sys.argv[1:]
checks = {
