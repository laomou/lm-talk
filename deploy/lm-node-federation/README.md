...
`./smoke-test.sh` checks that all three reverse-proxied nodes respond to `/health` and `/control/stats`, generates a signed smoke identity and ContactCard inside the node container, publishes and finds that ContactCard through node A DHT, exports a snapshot from node A, imports it into node B, and checks node B sync status.
